...
from tile_fetcher.services.models import GeoBBox, GeoPoint
...
class _FakeImageProvider:
    ...
    async def resolve_tile_for_point(
        self, gid: str, point: GeoPoint, timeout_seconds: float
    ) -> ImageTile:
        self.find_tile_calls.append((gid, point.lon, point.lat, timeout_seconds))
        return self.tile

    async def fetch_image(self, gid: str, pixel_bbox: PixelBBox, timeout_seconds: float) -> ProviderImage:
...
class _FakeProjectionMapper:
    ...
    async def geo_to_pixel_points(
        self, gid: str, points: Sequence[GeoPoint], timeout_seconds: float
    ) -> list[PixelPoint]:
        point = points[0]
        self.geo_to_pixel_calls.append((gid, point.lon, point.lat, timeout_seconds))
        return [self._points[gid]]

    async def pixel_to_geo_points(
        self, gid: str, points: Sequence[PixelPoint], timeout_seconds: float
    ) -> list[GeoPoint]:
        center = self._points[gid]
        output: list[GeoPoint] = []
        for point in points:
            ...
            if math.isclose(point.x, center.x + 1.0) and math.isclose(point.y, center.y):
                output.append(GeoPoint(lon=_degrees_for_meters(1.0), lat=0.0))
                continue
            if math.isclose(point.x, center.x) and math.isclose(point.y, center.y + 1.0):
                output.append(GeoPoint(lon=0.0, lat=_degrees_for_meters(1.0)))
                continue
...
class TileFetchServiceTests(unittest.IsolatedAsyncioTestCase):
    async def test_fetch_tiles_at_point_as_base64_uses_meter_buffer(self) -> None:
        ...
        provider = _FakeImageProvider(
            ...
            tile=ImageTile(
                image_id="img-1",
                bbox=GeoBBox(xmin=0.0, ymin=0.0, xmax=20.0, ymax=20.0),
                azimuth=15.0,
            ),
            find_tile_calls=[],
            fetch_image_calls=[],
    ...
    async def test_north_aligned_flag_changes_output(self) -> None:
        ...
        async def run(north_aligned: bool) -> str:
            ...
            projection_mapper = _FakeProjectionMapper(points={"img-1": PixelPoint(x=100.0, y=120.0)})
            service = TileFetchService(
                image_provider=ImageProviderClient(
                    resolve_tile_for_point=provider.resolve_tile_for_point,
                    fetch_image=provider.fetch_image,
